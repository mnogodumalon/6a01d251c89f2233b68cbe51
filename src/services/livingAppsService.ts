...
import type { PromptGeneratorPro, CreatePromptGeneratorPro } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getPromptGeneratorPro(): Promise<PromptGeneratorPro[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.PROMPT_GENERATOR_PRO}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as PromptGeneratorPro[];
    return enrichLookupFields(records, 'prompt_generator_pro');
  }
  static async getPromptGeneratorProEntry(id: string): Promise<PromptGeneratorPro | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.PROMPT_GENERATOR_PRO}/records/${id}`);
    const record = { record_id: data.id, ...data } as PromptGeneratorPro;
    return enrichLookupFields([record], 'prompt_generator_pro')[0];
  }
  static async createPromptGeneratorProEntry(fields: CreatePromptGeneratorPro) {
    return callApi('POST', `/apps/${APP_IDS.PROMPT_GENERATOR_PRO}/records`, { fields: cleanFieldsForApi(fields as any, 'prompt_generator_pro') });
  }
  static async updatePromptGeneratorProEntry(id: string, fields: Partial<CreatePromptGeneratorPro>) {
    return callApi('PATCH', `/apps/${APP_IDS.PROMPT_GENERATOR_PRO}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'prompt_generator_pro') });
  }
  static async deletePromptGeneratorProEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.PROMPT_GENERATOR_PRO}/records/${id}`);
  }
