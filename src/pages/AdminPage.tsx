...
const ENTITY_TABS = [
  { key: 'prompt_generator_pro', label: 'Prompt-Generator Pro', pascal: 'PromptGeneratorPro' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('prompt_generator_pro');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'prompt_generator_pro': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'prompt_generator_pro': return (data as any).promptGeneratorPro as PromptGeneratorPro[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'prompt_generator_pro': return PROMPTGENERATORPRO_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'prompt_generator_pro': return {
        create: (fields: any) => LivingAppsService.createPromptGeneratorProEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updatePromptGeneratorProEntry(id, fields),
        remove: (id: string) => LivingAppsService.deletePromptGeneratorProEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'prompt_generator_pro' || dialogState?.entity === 'prompt_generator_pro') && (
        <PromptGeneratorProDialog
          open={createEntity === 'prompt_generator_pro' || dialogState?.entity === 'prompt_generator_pro'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'prompt_generator_pro' ? handleUpdate : (fields: any) => handleCreate('prompt_generator_pro', fields)}
          defaultValues={dialogState?.entity === 'prompt_generator_pro' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['PromptGeneratorPro']}
          enablePhotoLocation={AI_PHOTO_LOCATION['PromptGeneratorPro']}
        />
      )}
      {viewState?.entity === 'prompt_generator_pro' && (
        <PromptGeneratorProViewDialog
          open={viewState?.entity === 'prompt_generator_pro'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'prompt_generator_pro', record: r }); }}
        />
      )}
