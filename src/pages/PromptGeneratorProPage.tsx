...
export default function PromptGeneratorProPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<PromptGeneratorPro[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<PromptGeneratorPro | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getPromptGeneratorPro());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: PromptGeneratorPro['fields']) {
  ...
  async function handleUpdate(fields: PromptGeneratorPro['fields']) {
    ...
    await LivingAppsService.updatePromptGeneratorProEntry(editingRecord.record_id, fields);
    await loadData();
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('rolle_persona')}>
                <span className="inline-flex items-center gap-1">
                  Rolle / Persona der KI
                  {sortKey === 'rolle_persona' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('kontext')}>
                <span className="inline-flex items-center gap-1">
                  Kontext / Ausgangssituation
                  {sortKey === 'kontext' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('aufgabe')}>
                <span className="inline-flex items-center gap-1">
                  Konkrete Aufgabe / Ziel
                  {sortKey === 'aufgabe' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('format_ausgabe')}>
                <span className="inline-flex items-center gap-1">
                  Format der Ausgabe
                  {sortKey === 'format_ausgabe' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/prompt-generator-pro/${record.record_id}`); }}>
